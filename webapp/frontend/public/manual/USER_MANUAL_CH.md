...
4. [模拟器](#4-模拟器)
5. [保存与导出](#5-保存与导出)
6. [导入外来过滤器（进阶）](#6-导入外来过滤器进阶)
7. [在游戏中安装过滤器](#7-在游戏中安装过滤器)
8. [小技巧与常见问题](#8-小技巧与常见问题)
...
- **页面标签** —— **编辑器**、**外观与音效**、**模拟器**、**保存与导出**、**导入过滤器**，即下文介绍的主页面。
...
不用再逐个阶级手调颜色 —— 让应用替你设计整条阶级梯。选中一个分类，点击 **🎛 色调生成器**，挑一个主色调，生成器立刻为该分类并排给出 **5 套完整的样式系列**：

![色调生成器](images/ch/05d-hue-generator.png)

| 系列 | 风格 |
|---|---|
| Sharket标准 | 本站默认风格 —— 深色底 + 主色文字/边框 |
| 浓重填充 | 每个阶级都是实色填充背景，醒目张扬 |
| 简约描边 | 完全无背景 —— 只有主色文字和细边框，干扰最小 |
| 浅色亮底 | 浅亮背景 + 深色主色文字（"白底标签"风格） |
| 霓虹 | 黑底上的高饱和发光色 |

- **自由混搭** —— 点击系列顶部的按钮应用整列，或点击单个格子按阶级组合不同系列（例如顶级用浓重填充、低级用简约描边）。
- **第二色调** —— 勾选*使用第二色调*后，高阶级用一种颜色、低阶级用另一种；**配比**滑块决定在哪个阶级交接。
- **平滑渐变** —— 勾选后两种色调沿阶级梯平滑过渡（而不是分段切换）。此时**配比**滑块控制过渡的中点位置，**渐变曲线 (γ)** 滑块控制过渡是柔和还是锐利。
- **应用所选样式**会把结果写入该分类的自定义覆盖（你设置的**提示音不会被改动**）—— 再用平时的 💾 按钮保存即可。

### 把当前外观另存为预设

外观预设编辑器顶部（导出/导入旁边）的 **💾 另存为预设** 按钮会把当前基础主题**连同你的所有修改**打包成一个新的命名预设。未修改的分类自动从基础主题复制，所以预设永远是完整的。保存后可以选择把基础主题切换为新预设 —— 显示效果完全一致，同时覆盖层会被清空，方便重新开始调整。
...
> ⚠️ **这不是推荐的使用方式。** 本页面允许你导入他人的 `.filter` 文件（NeverSink / FilterBlade / 手写均可）并直接编辑。即使重新导出为标准 `.filter`，导入的过滤器仍然**不在**我们的预设体系内 —— 我们的主题、色调生成器、物品悬浮信息以及模拟器的阶级匹配都不会对它生效，而且每个赛季的更新维护会更困难。建议优先在其他页面从零构建你自己的过滤器。仅在你确实需要微调某个第三方过滤器时才使用本功能。

![导入外来过滤器](images/ch/08-import-foreign.png)

点击 **选择 .filter 文件** 选择任意过滤器。应用会按其**原本的结构**解析，并忠实保留一切 —— 注释、被禁用的规则、规则顺序在重新导出后都会原样保留。

- **章节侧栏** —— 如果过滤器带有章节标记（如 NeverSink 的 `[[NNNN]]` 标题），它们会变成可点击的导航大纲；没有标记的过滤器则**按物品类别自动分组**，让你始终能够导航。
- **规则卡片** —— 每条规则显示 显示/隐藏 徽章（点击切换）、它的条件，以及游戏内标签的实时预览。
- **编辑条件** —— 点击任意条件即可以文本方式编辑，点 **×** 删除，或用 **+ 条件** 新增（例如 `ItemLevel >= 84`）。
- **编辑样式** —— **样式** 按钮会展开 文本 / 边框 / 背景 颜色选择器、字号滑块，以及**音效、小地图图标、光柱选择器**（与应用其他地方使用的相同）。
- **增删规则** —— **+ 规则** 在顶部新增一条；每张卡片都有 **＋**（在下方插入规则）和 **🗑**（删除）。
- **批量改样式** —— 点击 **选择**，勾选多条规则，然后一次性对它们应用 显示/隐藏、颜色，或 音效 / 图标 / 光柱。
- **导出 .filter** 会重新导出（编辑后的）过滤器，可直接放入游戏使用。

导入的过滤器会保存在你的浏览器中，直到你点击 **清除** 或导入另一个文件。

### 模拟器

点击 **▶ 模拟** 即可查看导入的过滤器如何渲染真实掉落。它会生成随机物品，并用**第一条命中的 显示/隐藏 规则**为每件物品上样式 —— 与游戏中完全一致。设置 **区域等级** 和 **数量**，然后点击 **生成掉落**。

![外来过滤器模拟器](images/ch/08b-import-simulator.png)

地面上至少保证约 30% 为可见掉落；其余为随机掉落，其中**被隐藏**的物品会以灰显方式呈现，方便你看到过滤器隐藏了哪些东西。**点击任意掉落物**可直接跳转到为其上样式的规则。

> 模拟器可以评估 基底、类别、稀有度、物品等级、插槽、堆叠数量 等条件。对于它无法模拟的条件（具体词缀、附魔）所限定的规则会被跳过，这些物品会落到它能够确认的规则上。
...
*本手册中的截图由脚本自动从线上站点抓取（见仓库中的 `docs/manual/capture_screenshots.mjs`）。界面更新后重新运行即可刷新截图。*
