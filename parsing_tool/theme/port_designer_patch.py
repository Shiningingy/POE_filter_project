...
def load_live_tiers():
    ...
    Hide tiers carry no style (Ruthless cannot Hide) and safety nets are not part of the
    authored band ladder — the designer gives nets one fixed look in their own section — so
    neither takes a position when pairing a band list against the ladder.
    """
...
def resolve_rows(section_keys, section, tcat, live):
    ...
    by_name = {tkey: rung for tkey, rung in ladder}
    pos = 0
    for k in section_keys:
        tkey = rung = None
        if not is_band_key(k):                       # `Tier N` names one of our tiers
            ...
            for name, r in ladder:
                if name == stem or name.startswith(stem + " "):
                    ...
                    break
            if tkey is None:
                notes.append((k, "names no tier in %r" % tcat))
                continue
        else:                                        # `RN` is a band — pair down the ladder
            if pos >= len(ladder):
                notes.append((k, "the ladder has only %d live tiers — nothing left to pair" % len(ladder)))
                continue
            tkey, rung = ladder[pos]
            pos += 1
        if rung is None:
