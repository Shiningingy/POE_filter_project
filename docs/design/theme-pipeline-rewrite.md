...
- The hue generator (`themeGenerator.ts`) becomes the preset *author* rather than an
  ...
  - **`custom_overrides.json` — NOT dead.** It has live `GET /api/custom-overrides` and
    `POST` endpoints (`main.py:785`, `:792`) and is plumbed through the backend, the
    Sidebar, `clientData`, `demoAdapter` and the bundle baker. It is `{}` because nobody
    has saved a per-category override yet, not because nothing reads it. Removing it
    would delete a capability, which is a decision, not a cleanup.
...
⚠️ **New argument AGAINST, created by workstream A.** `test_resolver_equivalence.mjs`
checks the editor preview against real emitted filter text, and its oracle is a fresh
`generate.py --trace` run. That only proves something because Python is an INDEPENDENT
implementation — now that the preview, the simulator and the TS generator all share
`filterStyle.ts`, comparing any of them to each other is close to tautological.

Delete Python and the equivalence test loses the only outside opinion in the project. So
G is no longer "delete the duplicate"; it is a trade between *one less thing to edit
twice* and *no independent check on the thing everything now depends on*. Decide it
deliberately, in the ADR, rather than as tidying.
